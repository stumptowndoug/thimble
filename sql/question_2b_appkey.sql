...
lag(sum(total_policies),1) over(partition by appkey order by coverage_month) as start_policies,
sum(new_policies) as new_policies,
sum(end_policies) as termed_policies,
ROUND(1.00 * (sum(total_policies) - sum(new_policies)) / lag(sum(total_policies),1) over(partition by appkey order by coverage_month),3) as retention
