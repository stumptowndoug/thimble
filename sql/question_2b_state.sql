...
select
coverage_month,
left(coverage_territory,2) as state,
sum(total_policies) as total_policies,
lag(sum(total_policies),1) over(partition by left(coverage_territory,2) order by coverage_month) as start_policies,
sum(new_policies) as new_policies,
sum(end_policies) as termed_policies,
ROUND(1.00 * (sum(total_policies) - sum(new_policies)) / lag(sum(total_policies),1) over(partition by left(coverage_territory,2) order by coverage_month),3) as retention
